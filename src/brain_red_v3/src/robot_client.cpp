...
int RobotClient::moveHead(double pitch, double yaw)
{
    ...
    brain->log->log("move_head", format("pitch: %.2f, yaw: %.2f", pitch, yaw));
...
int RobotClient::RLVisionKick(bool start)
{
    ...
    nlohmann::json body;
    body["start"] = start;
    // Match VisualKickParameter::ToJson(): firmware expects version; kV1=0, kV2=1.
    // Use ints (not VisualKickVersion) so older SDK headers without that enum still compile.
    // Invalid values fall back to kV2. T1 never enters VisualKick; callers still exit via start=false.
    const std::string vkVersion = brain->config->get_rl_vision_kick_version();
    body["version"] = (vkVersion == "kV1") ? 0 : 1;
    msg.body = body.dump();

...
int main(int argc, char **argv)
{
    ...
    auto game_control_sub = brain->create_subscription<game_controller_interface::msg::GameControlData>(
        "/robocup/game_controller", 10,
        bind(&Brain::gameControlCallback, brain, std::placeholders::_1));
    ...
    thread t1([&brain, &argc, &argv]() {
        ...
        opt.context(context);
        // Do not apply launch remapping to this helper node.
        opt.use_global_arguments(false);
        const std::string ext_name = std::string(brain->get_name()) + "_ext";
        auto node = rclcpp::Node::make_shared(ext_name, opt);

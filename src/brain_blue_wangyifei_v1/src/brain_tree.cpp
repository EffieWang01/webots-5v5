...
bool isAttackingRole(const string &role) {
    return role == "primary_striker" || role == "secondary_striker" || role == "midfielder" || role == "striker";
}

bool isSecondaryAttackRole(const string &role) {
    return role == "secondary_striker" || role == "midfielder";
}
...
int v3SetplayOffenseRank(Brain *brain, const string &realSubState, int costRank) {
    ...
    if (realSubState == "GOAL_KICK") {
        if (myId == 1 || isGoalkeeperRole(role)) return 0;
        return max(1, costRank + 1);
    }

    // Throw-in / corner / FK / PK: field attack roles take; rank among them by cost.
    ...
    for (int i = 0; i < HL_MAX_NUM_PLAYERS; ++i) {
        ...
        if (!isAttackingRole(tm.role)) continue;
...
void BrainTree::initEntry()
{
    ...
    setEntry<string>("assigned_role", brain->config->get_player_role());
    ...
    setEntry<int>("current_lead_id", 0);
    setEntry<int>("kickoff_taker_id", 0);
    setEntry<bool>("is_kickoff_taker", false);
    setEntry<int>("attack_rank", 0);
    setEntry<int>("defense_rank", 0);
    ...
    setEntry<bool>("shot_lane_open", true);
    setEntry<double>("shot_lane_clearance", 999.0);
    setEntry<double>("shot_target_y", 0.0);
    setEntry<double>("goalie_ball_vx", 0.0);
    setEntry<double>("goalie_ball_vy", 0.0);
    setEntry<double>("goalie_time_to_goal", 999.0);
    setEntry<double>("goalie_goal_intercept_y", 0.0);
    setEntry<bool>("goalie_predicted_shot", false);
...
NodeStatus GoToGoalBlockingPosition::tick() {
    ...
    Pose2D targetPose;
    const bool goalkeeperRole = isGoalkeeperRole(curRole);
    const bool defenderRole = curRole == "defender";
    const bool attackRole = isAttackingRole(curRole);
    const double ownGoalX = -fd.length / 2.0;
    const double ballFromGoal = std::max(0.25, ballPos.x - ownGoalX);

    if (goalkeeperRole) {
        // Active goalkeeper: stay close to the goal, but continuously slide along
        // the shot line.  The former 0.8 m arrival tolerance made the keeper look
        // frozen even when the ball moved by a visible amount.
        const double ballProgress = cap(ballFromGoal / (fd.length * 0.55), 1.0, 0.0);
        const double keeperDepth = 0.55 + 0.45 * (1.0 - ballProgress);
        targetPose.x = ownGoalX + keeperDepth;
        targetPose.y = ballPos.y * keeperDepth / ballFromGoal;
        const double keeperYLimit = std::max(0.35, fd.goalWidth / 2.0 - 0.20);

        // A fast shot is blocked at its predicted intersection with the keeper
        // plane, rather than by following the ball's current bearing.  The
        // decision node supplies a filtered velocity estimate through the
        // blackboard, so noisy single-frame jumps do not drag the keeper away.
        const bool predictedShot = brain->tree->getEntry<bool>("goalie_predicted_shot");
        const double ballVx = brain->tree->getEntry<double>("goalie_ball_vx");
        const double ballVy = brain->tree->getEntry<double>("goalie_ball_vy");
        if (predictedShot && ballVx < -0.05) {
            const double timeToKeeperPlane = (targetPose.x - ballPos.x) / ballVx;
            if (timeToKeeperPlane >= 0.0 && timeToKeeperPlane <= 2.60) {
                targetPose.y = ballPos.y + ballVy * timeToKeeperPlane;
            } else {
                targetPose.y = brain->tree->getEntry<double>("goalie_goal_intercept_y");
            }
        }
        targetPose.y = cap(targetPose.y, keeperYLimit, -keeperYLimit);
    } else if (defenderRole) {
        // Dynamic cover line: remain behind the ball instead of guarding one fixed
        // coordinate.  This makes the defender drop with an attack and step up
        // when possession moves away from our goal.
        const double minDefenseX = ownGoalX + 2.6;
        const double maxDefenseX = ownGoalX + 5.2;
        targetPose.x = cap(ballPos.x - 1.8, maxDefenseX, minDefenseX);

        const double defenseDepth = targetPose.x - ownGoalX;
        targetPose.y = ballPos.y * defenseDepth / ballFromGoal;
        targetPose.y = cap(targetPose.y,
                           fd.penaltyAreaWidth / 2.0 - 0.45,
                           -fd.penaltyAreaWidth / 2.0 + 0.45);

        // If a formation supplies two defenders, keep them from selecting the
        // same blocking point.  The nearer lane follows the ball; the other one
        // protects the opposite passing lane.
        int defenderCount = 1;
        int defenderOrdinal = 0;
        const int selfIdx = brain->config->get_player_id() - 1;
        for (int i = 0; i < HL_MAX_NUM_PLAYERS; ++i) {
            if (i == selfIdx || !brain->data->tmStatus[i].isAlive) continue;
            if (brain->data->tmStatus[i].role != "defender") continue;
            ++defenderCount;
            if (i < selfIdx) ++defenderOrdinal;
        }
        if (defenderCount > 1) {
            const double laneOffset = defenderOrdinal % 2 == 0 ? 0.85 : -0.85;
            targetPose.y = cap(targetPose.y + laneOffset,
                               fd.penaltyAreaWidth / 2.0 - 0.35,
                               -fd.penaltyAreaWidth / 2.0 + 0.35);
        }
    } else {
        // Preserve the original support/set-play behavior for attacking roles.
        targetPose.x = attackRole
            ? std::max(ownGoalX + distToGoalline, ballPos.x - 1.5)
            : ownGoalX + distToGoalline;
        if (ballFromGoal < distToGoalline) {
            targetPose.y = attackRole
                ? (ballPos.y > 0 ? fd.goalWidth / 2.0 : -fd.goalWidth / 2.0)
                : (ballPos.y > 0 ? fd.goalWidth / 4.0 : -fd.goalWidth / 4.0);
        } else {
            targetPose.y = ballPos.y * distToGoalline / ballFromGoal;
            targetPose.y = attackRole
    ...
    double vx = targetPose_r.x;
    double vy = targetPose_r.y;
    double vtheta = cap(brain->data->ball.yawToRobot * 2.5, vthetaLimit, -vthetaLimit);
...
NodeStatus Assist::tick() {
    ...
    for (int i = 0; i < HL_MAX_NUM_PLAYERS; i++) {
        ...
        if (tmStatus.isLead) {
            leadKnown = true;
            leadY = tmStatus.robotPoseToField.y;
            continue;
        }
        if (!isAttackingRole(tmStatus.role)) continue; 
    ...
    double lateral = 0.5;
    double rCrowd = 2.0;
    double runTriggerX = 0.0;
    double runAhead = 1.2;
    double farPostTrigger = 3.5;
    double farPostXMargin = 1.4;
    double farPostY = 1.2;
    if (v3Enabled(brain)) {
        ...
        brain->get_parameter("strategy.v3.secondary_run_trigger_x", runTriggerX);
        brain->get_parameter("strategy.v3.secondary_run_ahead", runAhead);
        brain->get_parameter("strategy.v3.secondary_far_post_trigger", farPostTrigger);
        brain->get_parameter("strategy.v3.secondary_far_post_x_margin", farPostXMargin);
        brain->get_parameter("strategy.v3.secondary_far_post_y", farPostY);
        if (curRole == "midfielder") {
            isSecondary = true;
            has2Assists = true;
        } else if (curRole == "secondary_striker") {
            isSecondary = true;
        }
    }

    Pose2D targetPose;
    const double opponentGoalX = fd.length / 2.0;
    // Occupy the lane opposite the current ball carrier. When the carrier is
    // central, jersey parity gives a stable side instead of lane oscillation.
    const double referenceY = leadKnown ? leadY : ballPos.y;
    const double laneSign = fabs(referenceY) > 0.15
        ? (referenceY >= 0.0 ? -1.0 : 1.0)
        : ((brain->config->get_player_id() % 2 == 0) ? 1.0 : -1.0);
    const bool secondaryForwardRun = v3Enabled(brain)
        && curRole == "secondary_striker"
        && ballPos.x >= runTriggerX;
    const bool secondaryFarPostRun = secondaryForwardRun
        && ballPos.x >= opponentGoalX - farPostTrigger;

    if (secondaryFarPostRun) {
        targetPose.x = min(opponentGoalX - farPostXMargin, ballPos.x + runAhead);
        targetPose.y = laneSign * farPostY;
        log("secondary far-post run");
    } else if (secondaryForwardRun) {
        targetPose.x = min(opponentGoalX - farPostXMargin, ballPos.x + runAhead);
        targetPose.y = ballPos.y + laneSign * lateral;
        log("secondary forward support run");
    } else {
        targetPose.x = isSecondary ? ballPos.x - secondaryBehind : ballPos.x - primaryBehind;
        targetPose.y = ballPos.y + (has2Assists || isSecondary ? laneSign * lateral : 0.0);
    }
    targetPose.x = max(targetPose.x, -fd.length / 2.0 + distToGoalline);
    targetPose.x = min(targetPose.x, opponentGoalX - 0.8);
    ...
    if (v3Enabled(brain)) {
        ...
        if (d < rCrowd) {
            ...
            // Ordinary support stays behind the ball. A deliberate attacking
            // run keeps its forward lane and separates laterally instead.
            if (!secondaryForwardRun && targetPose.x > ballPos.x - 0.5) {
...
NodeStatus CalcKickDir::tick()
{
    ...
    auto fd = brain->config->fieldDimensions;
    bool shotLaneOpen = true;
    double shotLaneClearance = 999.0;
    double shotTargetY = 0.0;
    ...
    if (brain->data->isFreekickKickingOff && !brain->data->isDirectShoot && !brain->isDefensing()) {
        ...
        brain->data->kickType = "shoot";
        const double goalX = fd.length / 2.0;
        const double goalTargetLimit = max(0.25, fd.goalWidth / 2.0 - 0.35);
        const double targetYs[3] = {-goalTargetLimit, 0.0, goalTargetLimit};
        const double robotTheta = brain->data->robotPoseToField.theta;
        const double currentBallDir = brain->data->robotBallAngleToField;

        double bestScore = -1e9;
        double bestDir = atan2(-bPos.y, goalX - bPos.x);
        double bestShotDistance = norm(goalX - bPos.x, bPos.y);
        for (double targetY : targetYs) {
            const double candidateDir = atan2(targetY - bPos.y, goalX - bPos.x);
            const double candidateRobotYaw = toPInPI(candidateDir - robotTheta);
            const double shotDistance = norm(goalX - bPos.x, targetY - bPos.y);
            const double obstacleDistance = brain->distToObstacle(candidateRobotYaw);
            const double clearance = min(shotDistance, obstacleDistance);
            const double alignmentPenalty = fabs(toPInPI(candidateDir - currentBallDir));
            // Prefer a genuinely clear lane, then the lane requiring less circling.
            const double score = min(clearance, 4.0) - 0.35 * alignmentPenalty
                - 0.03 * fabs(targetY);
            if (score > bestScore) {
                bestScore = score;
                bestDir = candidateDir;
                bestShotDistance = shotDistance;
                shotLaneClearance = clearance;
                shotTargetY = targetY;
            }
        }

        const double requiredClearance = min(3.0, max(1.20, bestShotDistance * 0.55));
        shotLaneOpen = shotLaneClearance >= requiredClearance;
        brain->data->kickDir = bestDir;

        // If all three goal lanes are blocked, look for an attacking teammate
        // with a usable passing corridor instead of repeatedly shooting into a body.
        if (!shotLaneOpen) {
            const int selfIdx = brain->config->get_player_id() - 1;
            double bestPassScore = -1e9;
            double bestPassDir = bestDir;
            bool passAvailable = false;
            for (int i = 0; i < HL_MAX_NUM_PLAYERS; ++i) {
                if (i == selfIdx || !brain->data->tmStatus[i].isAlive) continue;
                const auto &tm = brain->data->tmStatus[i];
                if (!isAttackingRole(tm.role)) continue;
                const double dx = tm.robotPoseToField.x - bPos.x;
                const double dy = tm.robotPoseToField.y - bPos.y;
                const double passDistance = norm(dx, dy);
                if (passDistance < 1.4 || passDistance > 6.0 || dx < -1.0) continue;
                const double passDir = atan2(dy, dx);
                const double passRobotYaw = toPInPI(passDir - robotTheta);
                const double passClearance = min(passDistance, brain->distToObstacle(passRobotYaw));
                const double requiredPassClearance = min(2.0, max(1.0, passDistance * 0.55));
                if (passClearance < requiredPassClearance) continue;
                const double passScore = passClearance + 0.20 * dx - 0.08 * passDistance;
            ...
            if (passAvailable) {
                brain->data->kickType = "cross";
                brain->data->kickDir = bestPassDir;
            }
        }
    ...
    if (v3Enabled(brain)) {
        ...
        if (lineSetPlay || (brain->tree->getEntry<string>("gc_game_sub_state_type") == "FREE_KICK"
            ...
            brain->data->kickType = "shoot";
            shotLaneOpen = true;
            shotLaneClearance = 999.0;
        }
    }

    brain->tree->setEntry<bool>("shot_lane_open", shotLaneOpen);
    brain->tree->setEntry<double>("shot_lane_clearance", shotLaneClearance);
    brain->tree->setEntry<double>("shot_target_y", shotTargetY);
    ...
    brain->log->log(
        "strategy/shot_lane",
        format("type=%s open=%d clearance=%.2f target_y=%.2f",
               brain->data->kickType.c_str(), shotLaneOpen,
               shotLaneClearance, shotTargetY)
    );
...
NodeStatus StrikerDecide::tick() {
    ...
    double deltaDir = toPInPI(kickDir - dir_rb_f);
    const bool shotLaneOpen = brain->tree->getEntry<bool>("shot_lane_open");
    const double shotLaneClearance = brain->tree->getEntry<double>("shot_lane_clearance");
    const auto fd = brain->config->fieldDimensions;
    const auto ballField = brain->data->ball.posToField;
    const bool inOpponentPenalty =
        ballField.x >= fd.length / 2.0 - fd.penaltyAreaLength - 0.35
        && fabs(ballField.y) <= fd.penaltyAreaWidth / 2.0 + 0.35;
    const bool quickShotReady =
        brain->data->kickType == "shoot"
        && inOpponentPenalty
        && shotLaneOpen
        && brain->data->ballDetected
        ...
        && fabs(ballYaw) < 0.75
        && fabs(deltaDir) < 0.55;
    // When boxed in very close to goal, permit a pressure shot through the best
    // of the three lanes instead of circling forever in Adjust.
    const bool pressureShotReady =
        brain->data->kickType == "shoot"
        && inOpponentPenalty
        && !shotLaneOpen
        && shotLaneClearance > 0.90
        && brain->data->ballDetected
        && ballRange < 0.70
        && ballX > 0.03
        && fabs(ballYaw) < 0.80
    ...
    if (!(iKnowBallPos || tmBallPosReliable))
    {
        ...
        newDecision = "assist";
    } else if (quickShotReady || pressureShotReady) {
        newDecision = "kick";
    } else if (ballRange > chaseRangeThreshold * (lastDecision == "chase" ? 0.9 : 1.0))
    {
        newDecision = "chase";
    } else if (
        ((brain->data->kickType == "shoot" && angleGoodForKick
        ...
        && ball.range < 1.5
        && ballX > 0.05
        && (brain->data->kickType != "shoot" || shotLaneOpen || pressureShotReady)
    ) {
...
NodeStatus DefenderDecide::tick()
{
    ...
    const auto ballPose = brain->data->ball.posToField;
    const double ownHalfLine = -0.15;  // slightly into own half on field X
    const bool inOwnHalf = ballX < ownHalfLine;
    const bool wasEngaged = lastDecision == "clear" || lastDecision == "chase"
        || lastDecision == "kick" || lastDecision == "adjust";
    // Engage at 3.0 m and keep a little hysteresis while already clearing.
    // Previously the 2.8 m branch required an existing chase decision, so a
    // stationary defender could only start moving once the ball was within 1.6 m.
    const double engageRange = wasEngaged ? 3.5 : 3.0;
    const double selfBallDist = norm(ballPose.x - selfPose.x, ballPose.y - selfPose.y);
    bool closestDefender = true;
    const int selfIdx = brain->config->get_player_id() - 1;
    for (int i = 0; i < HL_MAX_NUM_PLAYERS; ++i) {
        if (i == selfIdx || !brain->data->tmStatus[i].isAlive) continue;
        const auto &tm = brain->data->tmStatus[i];
        if (tm.role != "defender") continue;
        const double tmBallDist = norm(ballPose.x - tm.robotPoseToField.x,
                                       ballPose.y - tm.robotPoseToField.y);
        if (tmBallDist + 0.25 < selfBallDist) {
            closestDefender = false;
            break;
        }
    }
    const bool dangerNear = inOwnHalf && closestDefender && ballRange < engageRange;
    ...
    if (!(iKnowBallPos || tmBallPosReliable)) {
        ...
        // The closest defender actively clears; the dynamic blocking target keeps
        // it close enough for this branch to trigger without abandoning shape.
        if (ballRange < 0.55) {
            newDecision = "kick";
...
NodeStatus GoalieDecide::tick()
{
    ...
    double chaseRangeThreshold;
    getInput("chase_threshold", chaseRangeThreshold);
    const double chaseExitMargin = getInput<double>("chase_exit_margin").value();
    const double clearEnterRange = getInput<double>("clear_enter_range").value();
    const double clearExitRange = getInput<double>("clear_exit_range").value();
    const double shotMinSpeed = getInput<double>("shot_min_speed").value();
    const double shotMaxTime = getInput<double>("shot_max_time").value();
    string lastDecision, position;
    getInput("decision_in", lastDecision);
    ...
    bool angleIsGood = (dir_rb_f > -1.15 && dir_rb_f < 1.15);
    double ballRange = brain->data->ball.range;
    double ballYaw = brain->data->ball.yawToRobot;
    ...
    // Estimate ball velocity from fresh perception samples.  This project does
    // not expose ball velocity directly, so use a low-pass filtered finite
    // difference and reject localization jumps.  Each robot runs in its own
    // process, therefore this state belongs only to that robot's goalkeeper.
    static bool velocityInitialized = false;
    static Point lastBallPos{0.0, 0.0, 0.0};
    static rclcpp::Time lastBallTime(0, 0, RCL_ROS_TIME);
    static double filteredBallVx = 0.0;
    static double filteredBallVy = 0.0;
    const auto sampleTime = brain->data->ball.timePoint;
    const bool freshSample = sampleTime.nanoseconds() > 0
        && brain->msecsSince(sampleTime) <= 500.0;
    if (freshSample) {
        if (!velocityInitialized) {
            velocityInitialized = true;
            lastBallPos = brain->data->ball.posToField;
            lastBallTime = sampleTime;
        } else {
            const double dt = (sampleTime - lastBallTime).seconds();
            if (dt >= 0.04 && dt <= 0.80) {
                const double rawVx = (brain->data->ball.posToField.x - lastBallPos.x) / dt;
                const double rawVy = (brain->data->ball.posToField.y - lastBallPos.y) / dt;
                const double rawSpeed = norm(rawVx, rawVy);
                if (rawSpeed <= 9.0) {
                    const double alpha = 0.35;
                    filteredBallVx = alpha * rawVx + (1.0 - alpha) * filteredBallVx;
                    filteredBallVy = alpha * rawVy + (1.0 - alpha) * filteredBallVy;
                }
                lastBallPos = brain->data->ball.posToField;
                lastBallTime = sampleTime;
            } else if (dt > 0.80 || dt < 0.0) {
                filteredBallVx = 0.0;
                filteredBallVy = 0.0;
                lastBallPos = brain->data->ball.posToField;
                lastBallTime = sampleTime;
    ...
    const auto fdForPrediction = brain->config->fieldDimensions;
    const auto ballForPrediction = brain->data->ball.posToField;
    const double ownGoalLineX = -fdForPrediction.length / 2.0;
    double timeToGoal = 999.0;
    double goalInterceptY = ballForPrediction.y;
    bool predictedShot = false;
    if (freshSample && filteredBallVx < -shotMinSpeed) {
        timeToGoal = (ownGoalLineX - ballForPrediction.x) / filteredBallVx;
        goalInterceptY = ballForPrediction.y + filteredBallVy * timeToGoal;
        predictedShot = timeToGoal >= 0.0
            && timeToGoal <= shotMaxTime
            && fabs(goalInterceptY) <= fdForPrediction.goalWidth / 2.0 + 0.25;
    }
    brain->tree->setEntry<double>("goalie_ball_vx", filteredBallVx);
    brain->tree->setEntry<double>("goalie_ball_vy", filteredBallVy);
    brain->tree->setEntry<double>("goalie_time_to_goal", timeToGoal);
    brain->tree->setEntry<double>("goalie_goal_intercept_y", goalInterceptY);
    brain->tree->setEntry<bool>("goalie_predicted_shot", predictedShot);
    ...
    else
    {
        ...
        const double selfGoalX = -fd.length / 2.0;
        const double ownGoalAreaFrontX = selfGoalX + fd.goalAreaLength;
        const double ownPenaltyFrontX = selfGoalX + fd.penaltyAreaLength;

        const double goalAreaXMargin = 0.50;
        const double goalAreaYMargin = 0.50;
        const double penaltyXMargin = 0.35;
        const double penaltyYMargin = 0.40;
        const double maxOutXMargin = 0.50;

        const bool ballInGoalMouth =
            ballPos.x <= ownGoalAreaFrontX + goalAreaXMargin
            && fabs(ballPos.y) <= fd.goalAreaWidth / 2.0 + goalAreaYMargin;
        const bool ballInPenaltyCorridor =
            ballPos.x <= ownPenaltyFrontX + penaltyXMargin
            && fabs(ballPos.y) <= fd.penaltyAreaWidth / 2.0 + penaltyYMargin;
        const bool ballThreatensGoal = ballInGoalMouth || ballInPenaltyCorridor || predictedShot;
        const bool goalieTooFarOut = robotPose.x > ownPenaltyFrontX + maxOutXMargin;
        const bool rushEngaged = lastDecision == "chase"
            || lastDecision == "adjust" || lastDecision == "kick";
        const bool closeClearingNow = lastDecision == "adjust" || lastDecision == "kick";
        const double rushRange = chaseRangeThreshold + (rushEngaged ? chaseExitMargin : 0.0);
        const double closeClearRange = closeClearingNow ? clearExitRange : clearEnterRange;
        const bool ballMovingAway = filteredBallVx > 0.35;
        const bool robotOnGoalSide = robotPose.x <= ballPos.x - 0.05;
        const bool deepEnoughToRush = ballInGoalMouth
            || (ballInPenaltyCorridor && ballRange <= 1.50);

        if (predictedShot)
        {
            // Walking toward a fast incoming ball exposes the goal.  Stay on the
            // keeper plane and move laterally to the predicted intercept.
            newDecision = "block";
        }
        else if (!ballThreatensGoal)
        {
            newDecision = "retreat";
        }
        else if (goalieTooFarOut && ballRange > closeClearRange)
        {
            newDecision = "retreat";
        }
        else if (!deepEnoughToRush || ballMovingAway || ballRange > rushRange)
        {
            // Track the shooting line instead of being lured out by a slow,
            // lateral, or retreating ball.
            newDecision = "retreat";
        }
        else if (ballRange > closeClearRange)
        {
            newDecision = "chase";
        }
        else if (angleIsGood && robotOnGoalSide)
        {
            newDecision = "kick";
        }
        else
...
NodeStatus GoToReadyPosition::tick()
{
    ...
    if (role == "primary_striker" || role == "striker") {
        tx = isKickoff ? - fd.circleRadius - 0.5 : - fd.circleRadius * 2;
        ty = 0.0;
    } else if (role == "secondary_striker") {
        tx = isKickoff ? - fd.circleRadius - 0.8 : - fd.circleRadius * 2.2;
        ty = -1.5;
    } else if (role == "midfielder") {
        tx = - fd.circleRadius * 2.8;
        ty = isKickoff ? 1.2 : 0.8;
    } else if (isGoalkeeperRole(role)) {

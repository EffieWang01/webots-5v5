...
public:
    ...
    static BT::PortsList providedPorts()
    {
        return {
            InputPort<double>("chase_threshold", 1.0, "If the distance exceeds this value, execute the chase action"),
            InputPort<double>("chase_exit_margin", 0.45, "Extra distance allowed before leaving an active rush"),
            InputPort<double>("clear_enter_range", 0.72, "Enter close-ball clear mode inside this range"),
            InputPort<double>("clear_exit_range", 0.95, "Leave close-ball clear mode outside this range"),
            InputPort<double>("shot_min_speed", 0.20, "Minimum velocity toward own goal for shot prediction"),
            InputPort<double>("shot_max_time", 2.60, "Maximum predicted seconds to own goal line"),
...
/**
 * brain_v2: defender outputs find / defend / chase / adjust / kick.
 * defend uses GoToGoalBlockingPosition; clear sequence reuses Chase/Adjust/Kick.
 */
...
public:
    ...
    {
        return {
            InputPort<string>("decision_in", "", "Previous decision for hysteresis"),
            OutputPort<string>("decision_out"),
        };
    }
...
public:
    ...
    static BT::PortsList providedPorts()
    {
        return {
            ...
            InputPort<double>("vtheta_limit", 1.0, "turn speed limit"),

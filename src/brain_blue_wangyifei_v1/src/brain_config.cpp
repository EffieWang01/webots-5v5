...
void BrainConfig::handle()
{
    ...
    // Presets: adult_size=14×9 (practice), kid_size, robo_league=22×14 (5v5 competition).
    // Switch with game.field_type in config.yaml or ROS param — no rebuild needed.
    ...
    // Optional per-dimension overrides (>0 replaces preset). For local / odd pitches on deploy.
    auto *node = static_cast<rclcpp::Node *>(brain);
    auto apply_if = [&](const char *name, double &dst) {
        const double v = node->get_parameter_or(name, 0.0);
        if (v > 0.05) {
            dst = v;
        }
    };
    apply_if("game.field_length", fieldDimensions.length);
    apply_if("game.field_width", fieldDimensions.width);
    apply_if("game.field_penalty_dist", fieldDimensions.penaltyDist);
    apply_if("game.field_goal_width", fieldDimensions.goalWidth);
    apply_if("game.field_circle_radius", fieldDimensions.circleRadius);
    apply_if("game.field_penalty_area_length", fieldDimensions.penaltyAreaLength);
    apply_if("game.field_penalty_area_width", fieldDimensions.penaltyAreaWidth);
    apply_if("game.field_goal_area_length", fieldDimensions.goalAreaLength);
    apply_if("game.field_goal_area_width", fieldDimensions.goalAreaWidth);

...
Brain::Brain() : rclcpp::Node("brain_node")
{
    ...
    declare_parameter<double>("strategy.v3.r_crowd", 2.0);
    ...
    declare_parameter<double>("strategy.v3.assist_primary_behind", 3.5);
    declare_parameter<double>("strategy.v3.assist_secondary_behind", 5.0);
    declare_parameter<double>("strategy.v3.assist_lateral", 1.2);
...
void Brain::init()
{
    ...
    pubKickBall = create_publisher<brain_red_v3::msg::Kick>("/kick_ball" + topic_suffix, 10);
...
void Brain::handleCooperation() {
    ...
    double preferJerseyCost = 1e5;
    bool preferJerseyAlive = false;
    const bool v3Enable = get_parameter("strategy.v3.enable").as_bool();
    const int preferJersey = get_parameter("strategy.v3.lead_prefer_jersey").as_int();
    const int secondaryJersey = get_parameter("strategy.v3.lead_secondary_jersey").as_int();
    const int midfielderJersey = get_parameter("strategy.v3.lead_midfielder_jersey").as_int();
    const double secondaryMargin = get_parameter("strategy.v3.lead_secondary_margin").as_double();
    const double leadStableMs = get_parameter("strategy.v3.lead_stable_ms").as_double();
    const double ballStuckForceMs = get_parameter("strategy.v3.ball_stuck_force_chase_ms").as_double();
    for (int i = 0; i < aliveTmIdxs.size(); i++) {
        ...
        if (v3Enable && (tmIdx + 1) == preferJersey) {
            preferJerseyAlive = true;
            preferJerseyCost = tmStatus.cost;
        }
    }
    if (v3Enable && selfId == preferJersey) {
        preferJerseyAlive = true;
        preferJerseyCost = data->tmMyCost;
    }
    ...
    // Ball stuck: range barely changes for ball_stuck_force_ms → allow midfielder chase
    static double lastBallRangeForStuck = -1.0;
    static rclcpp::Time lastBallRangeChangeTime = get_clock()->now();
    static bool ballStillArmed = false;
    const bool ballKnown = tree->getEntry<bool>("ball_location_known") || tree->getEntry<bool>("tm_ball_pos_reliable");
    if (v3Enable && ballKnown && data->ball.range < 6.0 && data->ball.range > 0.05) {
        if (lastBallRangeForStuck < 0.0 || fabs(data->ball.range - lastBallRangeForStuck) > 0.25) {
            lastBallRangeForStuck = data->ball.range;
            lastBallRangeChangeTime = get_clock()->now();
            ballStillArmed = false;
        } else if (msecsSince(lastBallRangeChangeTime) > ballStuckForceMs) {
            ballStillArmed = true;
        }
    } else {
        ballStillArmed = false;
    ...
    if (v3Enable) {
        if (selfId == midfielderJersey && !ballStillArmed) {
            wantLead = false;
            log_("v3 midfielder: refuse lead");
        }
        // jersey 5 only steals lead from 4 when clearly closer
        if (selfId == secondaryJersey && preferJerseyAlive
            && data->tmMyCost + secondaryMargin >= preferJerseyCost) {
            wantLead = false;
            log_(format("v3 secondary: defer to jersey %d (margin)", preferJersey));
        }
    ...
    if (v3Enable) {
        static bool stableLead = true;
        static bool candidateLead = true;
        static rclcpp::Time candidateLeadSince = get_clock()->now();
        if (wantLead != candidateLead) {
            candidateLead = wantLead;
            candidateLeadSince = get_clock()->now();
        }
        // Gain lead slowly; drop lead faster to avoid dual-chase
        const double needMs = candidateLead ? leadStableMs : min(200.0, leadStableMs * 0.4);
        if (msecsSince(candidateLeadSince) >= needMs) {
            stableLead = candidateLead;
        }
        data->tmImLead = stableLead;
        tree->setEntry<bool>("is_lead", stableLead);
        log_(format("v3 lead want=%d stable=%d", wantLead ? 1 : 0, stableLead ? 1 : 0));
    } else if (wantLead) {
        data->tmImLead = true;
    ...
    // v3: who is the set-play taker (blackboard for BT + lead lock during line set-plays).
    {
        bool iAmTaker = false;
        const string subTypeNow = tree->getEntry<string>("gc_game_sub_state_type");
        const bool ownFreekick = v3Enable
            && subTypeNow == "FREE_KICK"
            && tree->getEntry<bool>("gc_is_sub_state_kickoff_side");
        const string roleNow = tree->getEntry<string>("player_role");
        const string realSub = data->realGameSubState;
        // Hysteresis is process-local (one Brain node per robot).
        static bool stickyLineTaker = false;
        static rclcpp::Time stickyLostSince = rclcpp::Time(0, 0, RCL_ROS_TIME);
        if (ownFreekick) {
            if (realSub == "GOAL_KICK") {
                stickyLineTaker = false;
                // Only the GK role takes — never jersey-1 striker mislabel.
                iAmTaker = (roleNow == "goal_keeper");
            } else if (roleNow == "striker") {
                int betterCount = 0;
                double bestOtherCost = 1e9;
                int bestOtherId = 99;
                for (int i = 0; i < aliveTmIdxs.size(); i++) {
                    int tmIdx = aliveTmIdxs[i];
                    if (data->tmStatus[tmIdx].role != "striker") continue;
                    if (data->tmStatus[tmIdx].cost < data->tmMyCost) ++betterCount;
                    else if (fabs(data->tmStatus[tmIdx].cost - data->tmMyCost) < 1e-6
                             && (tmIdx + 1) < selfId) {
                        // Equal cost: lower jersey id wins (prevent dual taker).
                        ++betterCount;
                    }
                    if (data->tmStatus[tmIdx].cost < bestOtherCost
                        || (fabs(data->tmStatus[tmIdx].cost - bestOtherCost) < 1e-6
                            && (tmIdx + 1) < bestOtherId)) {
                        bestOtherCost = data->tmStatus[tmIdx].cost;
                        bestOtherId = tmIdx + 1;
                    }
                }
                const bool rawTaker = (betterCount == 0);
                // Once taker, keep until clearly beaten (stops 3/4/5 flicker pile-up).
                const bool lineTake = (realSub == "THROW_IN" || realSub == "CORNER_KICK");
                if (lineTake) {
                    if (rawTaker) {
                        stickyLineTaker = true;
                        stickyLostSince = rclcpp::Time(0, 0, RCL_ROS_TIME);
                        iAmTaker = true;
                    } else if (stickyLineTaker) {
                        if (stickyLostSince.nanoseconds() == 0) {
                            stickyLostSince = get_clock()->now();
                        }
                        // Drop only after 700ms of not being rank-0, or if crushed by >1.5 cost.
                        if (data->tmMyCost > bestOtherCost + 1.5
                            || msecsSince(stickyLostSince) > 700.0) {
                            stickyLineTaker = false;
                            iAmTaker = false;
                        } else {
                            iAmTaker = true;
        ...
        // During own throw-in/corner, lead ≡ taker so StrikerDecide kick path is single-owner.
        // During own goal kick, only GK may lead-attack the mark.
        if (ownFreekick && (realSub == "THROW_IN" || realSub == "CORNER_KICK")) {
            data->tmImLead = iAmTaker;
            tree->setEntry<bool>("is_lead", iAmTaker);
        } else if (ownFreekick && realSub == "GOAL_KICK") {
            data->tmImLead = iAmTaker;
            tree->setEntry<bool>("is_lead", iAmTaker);
        }
    }
...
void Brain::updateCostToKick() {
    ...
    if (get_parameter("strategy.v3.enable").as_bool()) {
        ...
        if (!setplayCost) {
            const int pid = config->get_player_id();
            const int midId = get_parameter("strategy.v3.lead_midfielder_jersey").as_int();
            const int preferId = get_parameter("strategy.v3.lead_prefer_jersey").as_int();
            ...
            } else if (pid == preferId) {
                cost = max(0.0, cost - preferBonus);
                log_(format("v3 prefer jersey cost bonus: -%.1f", preferBonus));
            }
        }
    }

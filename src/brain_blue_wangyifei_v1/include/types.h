...
// adult_size: common practice / older HL pitch (14×9). Switch via game.field_type — no rebuild.
const FieldDimensions FD_ADULTSIZE{14, 9, 2.1, 2.6, 1.5, 3, 6, 1, 4};
// robo_league: official 5v5 competition pitch (22×14). Must match field_spec_5v5.md / sim.
const FieldDimensions FD_ROBOLEAGUE{22, 14, 3.5, 2.6, 2.0, 5.0, 8.0, 2.0, 5.0};
// Optional numeric overrides: game.field_length / field_width / … (see BrainConfig::handle).
...
struct TMStatus {
    string role = "not initialized"; // goalkeeper, defender, midfielder, primary_striker, secondary_striker
    int assignedRoleCode = 0;
    int leadId = 0;
    int availability = 0; // 0 unknown, 1 active, 2 temporarily missing, 3 unavailable
    bool isAlive = false;
